<template>
	<div class="designate">
		...
		<div class="search hxy-mb-25">
			<Form ref="searchForm" :model="searchForm" :label-width="100" @keydown.native.enter.prevent="handleSearch">
				<Row>
					...
					<Col :md="12" :lg="8" :xl="6" :xxl="6">
						<FormItem label="部门筛选" prop="deptId">
							<tree-select v-model="searchForm.deptId" :options="positionList" no-results-text="暂无数据" no-options-text="暂无数据" :show-count="true" placeholder="请选择部门" />
						</FormItem>
					</Col>
					<Col :md="12" :lg="8" :xl="6" :xxl="6">
						<FormItem label="指派人" prop="createBy">
							<Select v-model="searchForm.createBy" placeholder="请选择指派人" clearable filterable>
		...
		<div class="hxy-mb-25">
			<Button type="primary" ghost class="hxy-mr-10" @click="openTaskDialog">新增指派任务</Button>
		...
		<Modal
			...
			<Form :model="taskForm" ref="taskForm" :rules="rulesTask" label-position="left" :label-width="100">
						...
						<Row :gutter="10">
							<Col span="12">
								<FormItem label="部门" prop="deptId">
									<tree-select v-model="addTaskForm.deptId" :options="positionList" :show-count="true" no-results-text="暂无数据" placeholder="请选择部门" @select="selectVal" />
								</FormItem>
							</Col>
							<Col span="12">
				...
				<Row :gutter="10">
					...
					<!-- <Col span="24">
						<FormItem label="抄送人" prop="cc">
							<Select v-model="taskForm.cc" multiple :max-tag-count="5" :max-tag-placeholder="maxTag" placeholder="请选择抄送人" clearable filterable>
								<Option v-for="(val,index) in ccList" :value="val.id" :key="index">{{val.name}}</Option>
							</Select>
						</FormItem>
					</Col>
					<Col span="24">
						<FormItem label="抄送到群" prop="group">
							<Select v-model="taskForm.group" multiple :max-tag-count="5" :max-tag-placeholder="maxTag" placeholder="请选择抄送到群" clearable filterable>
								<Option v-for="(val,index) in ccList" :value="val.id" :key="index">{{val.name}}</Option>
							</Select>
						</FormItem>
					</Col> -->
...
export default {
	...
	mounted(){
		...
		// this.taskListTitle = this.taskListTitle.filter(col => col.key !== 'id')
	},
	methods: {
		// 新增指派任务-选择任务开始时间
		selectBeginTime(e) {
		...
		// 清空新增指派任务弹框
		resetForm() {
			this.taskForm.taskOrderId = null
		...
		selectVal(val) {
			...
			this.addTaskForm.deptId = val.id
		},
		// 添加任务
		addTask() {
			this.$refs['addTaskForm'].validate((valid) => {
				if(valid) {
					...
					this.addTaskData.push(
						{
							id: this.index,
							deptId: this.addTaskForm.deptId,
							belongName: this.addTaskForm.belongName,
		...
		selectChange(selection) {
			this.ids = selection.map(item => item.taskOrderId)
