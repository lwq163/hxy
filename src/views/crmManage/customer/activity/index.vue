<template>
  <div>
      ...
      <div>
          <Button class="hxy-mr-20" type="primary" ghost @click="addCustomer">新增客户</Button>
          ...
          <Button class="hxy-mr-20" @click="transfer">批量转移</Button>
          <Button class="hxy-mr-20" @click="transKey">栏目调整</Button>
          <Button class="hxy-mr-20" :disabled="selectBtn" @click="handleMain">客情维护</Button>
      </div>
      <div class="hxy-mt-20">
      ...
      <Modal :title="transferTit" :width="720" :mask-closable="false" :scrollable="true" v-model="transferMod">
        ...
        <Form :label-width="70" ref="transferPar" :model="transferPar">
            ...
            <div v-show="transferPar.belongUserId === '0'" class="hxy-mt-20" style="border: 1px solid #eee;padding:16px 16px 0;border-radius: 4px;">
                <approve ref="approve_transfer"></approve>
            </div>
        </Form>
    </Modal>
    ...
    <!-- 客情维护 -->
    <Modal title="客情维护" :width="720" :mask-closable="false" :scrollable="true" v-model="customerInforMaintenanceMod">
        <div slot="footer">
            <Button @click="customerInforMaintenanceMod=false">取消</Button>
            <Button type="primary" style="margin-left: 8px" @click="handleMaintenance('maintenanceRef')">确认</Button>
        </div>
        <Form ref="maintenanceRef" :label-width="80" :model="maintenance">
            <FormItem label="联系人" prop="crmCustomerContactId">
                <Select
                    v-model="maintenance.crmCustomerContactId"
                    filterable
                    :disabled='customerDisabled'
                    clearable
                    @on-change="hangleMaintainContactDetail"
                    >
                    <Option v-for="(item, index) in listMaster" :key="index" :value="item.id">{{item.contactName}}</Option>
                </Select>
            </FormItem>
            <FormItem label="特殊日期" prop="specialDate">
                <Select :disabled='customerDisabled' placeholder="" v-model="maintenance.specialDate">
                    <Option v-for="(item,index) in spectionDate" :key="index" :value="item">{{item}}</Option>
                </Select>
            </FormItem>
            <FormItem label="服务类型" prop="wishType">
                <Select :disabled='customerDisabled' placeholder="" v-model="maintenance.wishType">
                    <Option v-for="(item,index) in wishType" :key="index" :value="item.value">{{item.label}}</Option>
                </Select>
            </FormItem>
            <FormItem label="礼品列表">
                <Button class="hxy-mb-10" @click="handleAddGift">新增礼品</Button>
                <Table border :columns="giftTitle" :data="giftData"></Table>
            </FormItem>
            <FormItem label="其他备注" prop="remark">
                <Input v-model='maintenance.remark' />
            </FormItem>
    ...
    <!-- 维护记录 -->
    <Modal title="维护记录" :width="720" :mask-closable="false" :scrollable="true" v-model="maintainCustomerMod">
        <div>
            <p>维护记录</p>
            <Table border ref="paymentCollectionTable" :columns="maintainRecordTitle" :data="maintainRecordData" @on-selection-change="customerSelect"></Table>
        </div>
        <div>
            <p>礼品详情</p>
            <Table border ref="paymentCollectionTable" :columns="giftDetailTitle" :data="giftDetailData" @on-selection-change="customerSelect"></Table>
        </div>
    </Modal>
  </div>
</template>
...
associate, tagAdd, transfer, changeStatus, getCustomerByCreditCode, tagList, associateList, contact, contactList, 
listContactDetail, maintainContactList, maintainContactDetail, crmCustomerRelationshipWish } from '_s/customer'
export default {
    data() {
        return {
            maintenance: {}, // 客情维护
            ...
            addCustomerRules: {
                ...
                followUpPerson: [{ required: true,message: '此项不能为空'}],
                    ...
                    return h('div', [
                        h('Button', {
                            ...
                            on: {
                                click: () => {
                                    ...
                                    if (params.row.id) {
                                        listContactDetail(params.row.id).then(res => {
                                            if(res.code === 200) {
                        ...
                        h('Button', {
                            ...
                            on: {
                                click: () => {
                                    ...
                                    if (params.row.id) {
                                        this.contactsBasicInfoForm.id = params.row.id // 编辑的联系人id
                                        listContactDetail(params.row.id).then(res => {
                                            if(res.code === 200) {
                                                this.contactsBasicInfoForm = res.data
                        ...
                        h('Button', {
                            ...
                            on: {
                                click: () => {
                                    if (params.row.id) {
                                        contact(params.row.id).then(res => {
                                            if(res.code === 200) {
                ...
                render: (h, params) => {
                    return h('Select', {
                        props: {
                            ...
                            disabled: this.addContactsAbled
                        },
                        on: {
                            'on-change':(val) => {
                ...
                render: (h,params) => {
                    return h('Input', {
                        props: {
                            ...
                            disabled: this.addShareholderInfoAble
                        },
                        on: {
                            input: (val) => {
            ...
            creditCode: '',
            // 客情维护
            customerInforMaintenanceMod: false,
            maintainCustomerMod: false,
            maintainRecordTitle: [{}],
            maintainRecordData: [],
            giftDetailTitle: [{}],
            giftDetailData: [],
            ...
            giftTitle: [{
                ...
                render: (h,params) => {
                    return h('Input', {
                        props: {
                            transfer: true,
                            value: params.row.giftName
                        },
                        on: {
                            input :(val) => {
                                this.giftData[params.index].giftName = val                                           
                            } 
                        },
                    })
                }
                ...
                render: (h, params) => {
                    return h('Input', {
                        props: {
                            transfer: true,
                            value: params.row.giftNums
                        },
                        on: {
                            input :(val) => {
                                this.giftData[params.index].giftNums = val                                                 
                            } 
                        },
                    })
                 }
                ...
                render: (h, params) => {
                    return h('div', [
                        h('Button', {
                            ...
                            on: {
                                click: () => {
                                    this.giftData.splice(params.index, 1)
                                    // this.getTag()
                                }
                            }
                        }, '删除')
    ...
    methods: {
		...
		async initApprove(){
		    ...
		    this.$refs['approve_transfer'].setData_mount(this.queryCurrStaff)
			await getCrmTemp({ formName: 5 }).then(res => { // 审批客情维护
				if (res.code === 200) {
					this.$refs['approve_ref'].setData_init(res);
				}
			})
            await getCrmTemp({ formName: 4 }).then(res => { // 审批转移客户
				if (res.code === 200) {
					this.$refs['approve_transfer'].setData_init(res);
				}
			})
		},
        // 新增礼品
        ...
        handleMaintenance() {
            ...
            this.maintenance.approvalTempId = roveInfo.approvalTempId,
            this.maintenance.approvalUserIds = roveInfo.approvalUserIds,
            this.maintenance.examineUserIds = roveInfo.examineUserIds,
            this.maintenance.ccUserIds = roveInfo.ccUserIds,
            this.maintenance.leaveMessage = roveInfo.leaveMessage
            this.maintenance.gifts = this.giftData
            crmCustomerRelationshipWish(this.maintenance).then(res => {
                if (res.code === 200) {
                    this.customerInforMaintenanceMod = false
                }
            })
        },
        ...
        chargeFn(params) {
            this.$Modal.confirm({
                title: "是否废弃该客户信息",
                onOk: () => {
                    changeStatus({ id: [params.id], customerStatus: 0}).then(res => {
                        if(res.code === 200){
                            this.getList()
        ...
        handleTransfer(id) {
            ...
            if (this.transferPar.belongUserId === '-1') { // 转移到直属上级
                ...
                //校验审批
                var roveInfo =  this.$refs.approve_transfer.check_role();
                params = {
                    ids: this.ids,
        ...
        addRelated() {
            ...
            if (this.user) {
                params = {
                    ...
                    createByName: this.user.nickName,
        ...
        handleContactAdd(name) {
            this.$refs[name].validate(valid => {
                if (valid) {
                    ...
                    let contactSexName,contactTypeName // 性别，类型
                    this.sysUserSexOpt.forEach(item => {
                        if (item.value === this.contactsBasicInfoForm.contactSex) {
                            contactSexName = item.label
                        }
                    })
                    this.contactTypeOpt.forEach(item => {
                        if (item.value === this.contactsBasicInfoForm.contactType) {
                            contactTypeName = item.label
                        }
                    })

                    let params = {
                        id: this.contactsBasicInfoForm.id ? this.contactsBasicInfoForm.id : '',
                        contactName: this.contactsBasicInfoForm.contactName,
                        contactSex: this.contactsBasicInfoForm.contactSex,
                        contactSexName: contactSexName,
                        ...
                        listHobby: this.listHobbyData,
                        listSpecialDate: this.listSpecialData
                    }
                    ...
                    // 新增/编辑时保存客户联系人
                    if (this.addContactsType === 0) { // 新增客户联系人
                        this.listContact.push(params)
                    } else if (this.addContactsType === 1) { // 编辑客户联系人
                        this.listContact = this.listContact.map(item => item.id === this.contactsBasicInfoForm.id ? params : item)
                    }
        ...
        // 请求客户联系人表格
        getContactList(id) {
            contactList({ crmCustomerId: id }).then(res => {
                if (res.code === 200) {
                    this.listContact = res.rows
                }
            })
        },
        // 添加决策人按钮
        ...
        // 彈出新增客戶框
        addCustomer() {
            this.$refs.addCustomerPar.resetFields()
            this.addCustomerPar = {}
            this.dialogTit = '新增客户',
            this.dialogType = 0,
            this.dialogMod = true
        ...
        handleMain() {
            this.maintenance = {}
            this.giftData = []
            this.customerInforMaintenanceMod = true
            this.getMaintainContactList()
        },
        // 客情维护联系人获取特殊日期你详情
        hangleMaintainContactDetail() {
            if(this.maintenance.crmCustomerContactId) {
               maintainContactDetail(this.maintenance.crmCustomerContactId).then(res => {
                   if (res.code === 200) {
                       this.spectionDate = res.data.listSpecialDate.map(item => { // 特殊日期
                           return item.specialDate
                       })
                   }
               })
        ...
        // 客情维护获取联系人
        getMaintainContactList() {
            maintainContactList().then(res => {
                if (res.code === 200) {
                    this.listMaster = res.rows
                }
            })
        }, 
        // 以下是穿梭框逻辑
